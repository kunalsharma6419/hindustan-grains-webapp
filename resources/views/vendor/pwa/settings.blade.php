                    ...
                    <form method="POST" action="{{ route('pwa.update') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="text" name="name"
                                    ...
                                    value="{{ $pwa->data['manifest']['name'] ?? '' }}" required>
                                @if ($errors->has('name'))
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="text" name="short_name"
                                    ...
                                    value="{{ $pwa->data['manifest']['short_name'] ?? '' }}" required>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="text" name="start_url"
                                    class="form-control {{ $errors->has('start_url') ? ' is-invalid' : '' }}"
                                    value="{{ $pwa->data['manifest']['start_url'] ?? '' }}" required>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="color" name="background_color" class="form-control"
                                    value="{{ $pwa->data['manifest']['background_color'] ?? '' }}" required>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="color" name="theme_color" class="form-control"
                                    value="{{ $pwa->data['manifest']['theme_color'] ?? '' }}" required>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-10">
                                <select name="display" class="form-control" required>
                                    <option value="standalone"
                                        {{ $pwa->data['manifest']['display'] == 'standalone' ? 'selected=selected' : false }}>
                                        Standalone</option>
                                    <option value="fullscreen"
                                        {{ $pwa->data['manifest']['display'] == 'fullscreen' ? 'selected=selected' : false }}
                                        disabled="disabled">Fullscreen</option>
                                    <option value="minimal-ui"
                                        {{ $pwa->data['manifest']['display'] == 'minimal-ui' ? 'selected=selected' : false }}
                                        disabled="disabled">Minimal UI</option>
                                    <option value="browser"
                                        {{ $pwa->data['manifest']['display'] == 'browser' ? 'selected=selected' : false }}
                                        disabled="disabled">Browser</option>
                                </select>
                                <p class="mt-3">For more details <a href="https://web.dev/add-manifest/#display">click
                                        here</a></p>
                            </div>
                        </div>
                        ...
                        <div class="ibox collapsed">
                            <div class="ibox-title d-flex pwa-title-box">
                                <h3 class="pwa-title"><strong>Icons</strong> <span>(All icons must be png format)</span>
                                </h3>
                            ...
                            <div class="ibox-content" style="display: block;">
                                @foreach ($pwa->data['manifest']['icons'] as $key => $icon)
                                    <div class="form-group row">
                                        <label class="col-lg-2 col-form-label pwa-label">{{ $key }}
                                            <span>*</span></label>
                                        <div class="col-10">
                                            <div class="input-group mb-3">
                                                <div class="custom-file">
                                                    <input type="file" class="custom-file-input"
                                                        id="{{ $key }}" name="icons[{{ $key }}]"
                                                        data-default_placeholder="{{ __('pwa.change-icon') }}"
                                                        accept="image/x-png">
                                                    <label class="custom-file-label" for="{{ $key }}"
                                                        aria-describedby="{{ $key }}">{{ __('pwa.change-icon') }}</label>
                                                </div>
                                            </div>
                                            <img src="{{ $icon['path'] }}" alt="{{ $key }}"
                        ...
                        <div class="ibox collapsed">
                            <div class="ibox-title d-flex pwa-title-box">
                                <h3 class="pwa-title"><strong>Splashes</strong> <span>(All splashes must be png
                                        format)</span></h3>
                                <div class="ibox-tools pwa-title-tools">
                                    <a class="collapse-link"><i class="fa fa-chevron-down"></i></a>
                                </div>
                            ...
                            <div class="ibox-content" style="display: block;">
                                @foreach ($pwa->data['manifest']['splash'] as $splash => $path)
                                    <div class="form-group row">
                                        <label class="col-lg-2 col-form-label pwa-label">{{ $splash }}
                                            <span>*</span></label>
                                        <div class="col-10">
                                            <div class="input-group mb-3">
                                                <div class="custom-file">
                                                    <input type="file" class="custom-file-input"
                                                        id="{{ $splash }}" name="splashes[{{ $splash }}]"
                                                        data-default_placeholder="{{ __('pwa.change-splash') }}"
                                                        accept="image/x-png">
                                                    <label class="custom-file-label" for="{{ $splash }}"
                                                        aria-describedby="{{ $splash }}">{{ __('pwa.change-splash') }}</label>
                                                </div>
                                            </div>
                                            <img src="{{ $path }}" alt="{{ $splash }}"
                                                style="max-width:100px;">
                                        </div>
                                    </div>
                                @endforeach
